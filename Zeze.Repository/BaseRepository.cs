...
using Zeze.Domin.Models;
using Zeze.IRepository;

namespace Zeze.Repository
{
    ...
    public abstract class BaseRepository<TEntity, TPrimaryKey> : IBaseRepository<TEntity, TPrimaryKey> where TEntity : BaseEntity
    {
        ...
        internal readonly BaseContext  _baseContext;
        ...
        {
            var dbEntityEntry = _baseContext.Entry<TEntity>(entity);
            if (properties.Any())
            {
        ...
        {
            _baseContext.ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking;
            try
            {
                int excuter = 0;
        ...
        public async Task<int> UpdateNotQueryAsync(TEntity entity, params Expression<Func<TEntity, object>>[] properties)
        {
            ...
            return await _baseContext.SaveChangesAsync();
        }
